...
*/
    ...
    @PreAuthorize("hasAuthority('admin')")
    @RequestMapping(value = "/downloadApplicantsStatusAndBankTransferData", method = RequestMethod.GET)
    public void downloadApplicantsStatusAndBankTransferData(Principal principle,
                                         HttpServletResponse response, String passcode, String passwd) throws IOException, SQLException {

        if (isAuthorizedForReports(principle, passcode, passwd)) {
            downloadDataAsCSV(response,
                    "SELECT concat(EDUAPPLICATION.FIRST_NAME, ' ', EDUAPPLICATION.LAST_NAME) as \"STUDENT NAME\", " +
                            "  EDUAPPLICATION.STUDENT_ID, " +
                            "  EDUAPPLICATION.CONFIRMATION_NMBR, " +
                            "  a.PROCESSING_STATUS, " +
                            "  a.AWARD_AMOUNT, " +
                            ...
                            "             ELSE concat( EDUAPPLICATION.CITY, ' ', EDUAPPLICATION.PIN)  END ADDRESS_LINE3, " +
                            "  EDUAPPLICATION.EMAIL, " +
                            "  a.P_BENEFICIARY_NAME as BENEFICIARY_NAME, " +
                            "  a.P_BANK_NAME as BANK_NAME, " +
                            "  a.P_BRANCH_IFSC_CODE as BRANCH_IFSC_CODE, " +
                            "  EDUAPPLICATION.BRANCH_NAME, " +
                            "  a.P_BANK_SWIFT_CODE as BANK_SWIFT_CODE, " +
                            "  a.P_BRANCH_ADDRESS_LINE1 as BRANCH_ADDRESS_LINE1, " +
                            "  a.P_BRANCH_ADDRESS_LINE2 as BRANCH_ADDRESS_LINE2, " +
                            "  a.P_BRANCH_ADDRESS_LINE3 as BRANCH_ADDRESS_LINE3 " +
                            "  from  EDUAPP_PROCESS_DETAIL a " +
                            " inner join EDUAPPLICATION on a.EDUAPP_ID = EDUAPPLICATION.ID " +
                            " where EDUAPPLICATION.APPLICATION_YEAR = (SELECT APP_YEAR FROM EDUAPP_CONFIG WHERE REGION ='" +
                            scholarshipOriginationService.getScholarshipOriginationRegion() + "') " +
                            " and EDUAPPLICATION.REGION = '" + scholarshipOriginationService.getScholarshipOriginationRegion() + "'" +
                            " order by 1",
                    "All-Applicants-Status-And-Banking-details");
        } else {
            downloadDataAsCSV(response, "SELECT ''", "blank");
        }
